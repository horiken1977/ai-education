...
> **⚠ この節は後の決定で変更されました。** 受講者の進捗データを非公開にするアクセス制御要件のため、ダッシュボードは**公開サイトから撤去し、Microsoft 365で非公開管理**に変更しています。最新方針は [ダッシュボード（M365・非公開）](dashboard-m365.md) と [講義モード設計](lecture-mode.md) §3 を参照。以下はGitHub完結を試みた旧案として残します。
