...
> **最新仕様**：ダッシュボードは **Forms書き出しの生CSV（`dashboard/data/responses.csv`）をJSが集計**する方式に変更（Forms設計・運用手順は [ダッシュボード](../dashboard/index.md) が正）。以下はGitHub完結の考え方の説明（旧サンプルは集計済みCSVだが、現行は生CSVをそのまま上書きするだけ）。
